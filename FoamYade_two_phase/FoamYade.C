...
void Foam::FoamYade::setRhoP(scalar _rhoP)
{
	rhoP = _rhoP; 
}

// void Foam::FoamYade::setScalar1Properties(scalar _rhoP, volScalarField _rho, volScalarField _nu){
// 	rhoP = _rhoP; rho = _rho; nu = _nu;
// }
...
void Foam::FoamYade::setCellVolFraction(){
	...
	for (const auto& iv : pVolcontrib) {
		...
		pFraction[cellId] = (vfrc > 0.10 && vfrc < 1.0 ) ? vfrc : 0.10;
		uParticle[cellId] = pVel/(1.0 - pFraction[cellId]);
...
void Foam::FoamYade::hydroDragForce(YadeParticle*  prt){
	...
	double rhof(0);
	double nuf(0);
	double pFraction_f = 0.0;
	double pv = 0.0;

	for (const auto& idwt : prt->interpCellWeight){
		uf += (U[idwt.first]*idwt.second);
		nuf += (nu[idwt.first]*idwt.second);
		rhof += (rho[idwt.first]*idwt.second);
		pFraction_f += (pFraction[idwt.first]*idwt.second);
	...
	if (pFraction_f > 0.8){
		...
		const double& pFraction_pSq = std::pow(pFraction_p, 2);
		const double& dpSq = std::pow(prt->dia, 2);
		double cf1 = 150*mu_f*(pFraction_pSq)*(1/(pFraction_f*dpSq));
	...
	for (const auto& idwt : prt->interpCellWeight){
		double ooCellVol = (prt->vol)/(rho[idwt.first]*mesh.V()[idwt.first]);
...
void Foam::FoamYade::addedMassForce(YadeParticle* prt){
	...
	for (unsigned int i=0; i != prt->interpCellWeight.size(); ++i) {
		const double& wt = prt->interpCellWeight[i].second;
		const int& id = prt-> interpCellWeight[i].first;
		const double& ooCellVol = 1./(mesh.V()[id]*rho[id]);
...
void Foam::FoamYade::archimedesForce(YadeParticle* prt) {
	...
	for (unsigned i =0 ; i != prt->interpCellWeight.size(); ++i) {
		...
		const double& wt = prt->interpCellWeight[i].second;
		pv +=  (prt->vol*wt);
		divt = divt + (2.0*nu[cellId]*divT[cellId]*wt*rho[cellId]);
		pg   =  pg + (gradP[cellId]*rho[cellId]*wt);
	}
	...
	for (unsigned i = 0; i != prt->interpCellWeight.size(); ++i){
		const int& cellId = prt->interpCellWeight[i].first;
		const double& wt = prt->interpCellWeight[i].second;
		const double& ooCellVol = 1./(mesh.V()[cellId]*rho[cellId]);
...
void Foam::FoamYade::stokesDragForce(YadeParticle* prt) {
	...
	const double& coeff = 3*M_PI*(prt->dia)*nu[prt->inCell]*rho[prt->inCell];
	const double& ooCellVol = 1./(mesh.V()[prt->inCell]*rho[prt->inCell]);
...
void Foam::FoamYade::stokesDragTorque(YadeParticle* prt) {
	...
	prt->hydroTorque = M_PI*(pow(prt->dia, 3))*(wfluid-prt->rotationalVelocity)*nu[prt->inCell]*rho[prt->inCell];
}
...
// 		uSourceDrag[cellId] = pFraction[cellId]*uSourceDrag[cellId];
// 		uSource[cellId] = pFraction[cellId]*uSource[cellId];
// 	}
// }
...
void Foam::FoamYade::updateSources() {
	...
	for (const auto& iv : pVolcontrib) {
		const int& cellId = iv.first;
		const double pFractionf = pFraction[cellId]; const double pFractionp = 1-pFraction[cellId];
		uCoeff[cellId] = mag(U[cellId]) > small ? (U[cellId] & uInterp[cellId])/(mag(U[cellId])) : 0.0;
		uSourceDrag[cellId] = uSourceDrag[cellId]/(pFractionp*pFractionf);
...
void Foam::FoamYade::calcHydroTorque(YadeProc* yProc){
	for (auto & prt : yProc->foundParticles) {
		if (isGaussianInterp) {
			...
			prt->hydroTorque += M_PI*(pow(prt->dia, 3))*(wfluid-prt->rotationalVelocity)*nu[prt->inCell]*rho[prt->inCell];// must check whether incell is correct idxing
		} else {
